...
then the chunk count as a varint, then that many 32 byte hashes.
...
On the loss of the primary, the cluster promotes the lowest indexed live metadata
node and reconciles it by adopting the log of the most advanced live node. A
committed operation was logged on a quorum of metadata nodes, so with the default
of three metadata nodes and a quorum of two the namespace survives the loss of
one metadata node. The `metadata_survives_primary_crash` test exercises this
path directly.
